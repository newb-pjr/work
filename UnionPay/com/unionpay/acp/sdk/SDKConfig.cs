...
namespace com.unionpay.acp.sdk
{
    ...
    public class SDKConfig
    {
        ...
        private static string signCertPwd = Security.DecryptDES(config.AppSettings.Settings["sdk.signCert.pwd"].Value, Security.theKey);//功能：读取配置文件获取签名证书密码
        ...
        private static string jfFrontTransUrl = "http://" + CommonVar.Yuming + "/" + config.AppSettings.Settings["sdk.jf.frontTransUrl"].Value; //功能：读取配置文件获取缴费产品前台交易地址
        private static string jfBackTransUrl = "http://" + CommonVar.Yuming + "/" + config.AppSettings.Settings["sdk.jf.backTransUrl"].Value;//功能：读取配置文件获取缴费产品后台交易地址
